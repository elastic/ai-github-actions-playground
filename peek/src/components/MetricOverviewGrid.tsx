...
export default function MetricOverviewGrid({
  ...
  useEffect(() => {
    ...
    const runBatches = async () => {
      // Mark all as loading
      const initialResults: Record<string, MetricCardResult> = {};
      for (const m of namespaceMetrics) {
        initialResults[m.name] = { status: "loading" };
      }
      setResults(initialResults);
      for (let i = 0; i < namespaceMetrics.length; i += BATCH_SIZE) {
        if (signal.aborted) return;
        const batch = namespaceMetrics.slice(i, i + BATCH_SIZE);
  ...
  // Filter to only metrics with data points
  const metricsWithData = useMemo(() => {
    return namespaceMetrics.filter((m) => {
      const r = results[m.name];
      return r?.status === "success" && r.data && r.data.values.length > 0;
    });
