...
Switch between List, Scatter, and Timeseries view modes using the chips above the results panel. List shows a table of matching traces. Scatter plots trace duration over time, colored by service, and supports clicking a point to open the trace detail. Timeseries view requires an aggregation query.

Click any row in the List view or any point in the Scatter view to load the trace detail panel below. The detail panel shows a waterfall chart of spans for the selected trace.
