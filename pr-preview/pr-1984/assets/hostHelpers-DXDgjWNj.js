...
| SORT bucket ASC`}function j(e,t="30 seconds"){const n=[`@timestamp >= ${e.timeFrom}`,`@timestamp <= ${e.timeTo}`];if(e.osType&&e.osType!=="unknown"){const s=e.osType==="macos"?"darwin":e.osType;n.push(`os.type == "${s}"`)}if(e.search){const s=h(e.search);n.push(`host.name LIKE "*${s}*"`)}return`FROM metrics-hostmetricsreceiver*
| WHERE ${n.join(" AND ")}
...
| STATS
    ...
    load_15m = AVG(system.cpu.load_average.15m)
  BY bucket
| SORT bucket ASC`}function X(e){if(!e?.columns||!e.values)return[];const t=e.columns.findIndex(s=>s.name==="bucket"),n=e.columns.findIndex(s=>s.name==="metric_value");return t<0||n<0?[]:e.values.map(s=>({bucket:String(s[t]??""),value:typeof s[n]=="number"?s[n]:0})).filter(s=>s.bucket)}function B(e){if(!e?.columns||!e.values)return[];const t=e.columns.findIndex(o=>o.name==="bucket"),n=e.columns.findIndex(o=>o.name==="load_1m"),s=e.columns.findIndex(o=>o.name==="load_5m"),r=e.columns.findIndex(o=>o.name==="load_15m");return t<0?[]:e.values.map(o=>({bucket:String(o[t]??""),load1m:typeof o[n]=="number"?o[n]:0,load5m:typeof o[s]=="number"?o[s]:0,load15m:typeof o[r]=="number"?o[r]:0})).filter(o=>o.bucket)}function a(e,t){return e.findIndex(n=>n.name===t)}function y(e,t){if(t<0||t>=e.length)return"";const n=e[t];return typeof n=="string"?n:n!=null?String(n):""}function A(e,t){if(t<0||t>=e.length)return null;const n=e[t];return typeof n=="number"&&Number.isFinite(n)?n:null}function G(e){const t=e.columns??[],n=e.values??[];if(t.length===0||n.length===0)return[];const s=a(t,"host_key"),r=a(t,"host_name"),o=a(t,"os_type"),u=a(t,"os_name"),m=a(t,"os_version"),l=a(t,"last_seen"),d=a(t,"cpu_utilization"),_=a(t,"memory_utilization"),p=a(t,"process_count"),i=a(t,"host_ip");return n.map(c=>({hostId:y(c,s)||"unknown",hostName:y(c,r),osType:v(y(c,o)),osName:y(c,u),osVersion:y(c,m),lastSeen:y(c,l),cpuUtilization:A(c,d),memoryUtilization:A(c,_),processCount:A(c,p),hostIp:y(c,i)||void 0}))}export{M,D as a,H as b,j as c,q as d,X as e,w as f,F as g,G as h,Q as i,W as j,z as k,L as o,B as p,N as t,V as u};
//# sourceMappingURL=hostHelpers-DXDgjWNj.js.map
